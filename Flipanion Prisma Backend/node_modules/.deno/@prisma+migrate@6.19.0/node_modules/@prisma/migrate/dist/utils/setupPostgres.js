...
__export(setupPostgres_exports, {
  runQueryPostgres: () => runQueryPostgres,
...
async function setupPostgres(options) {
  ...
  await dbDefault.end();
  if (dirname !== "") {
    const migrationScript = await import_promises.default.readFile(import_node_path.default.join(dirname, "setup.sql"), { encoding: "utf-8" });
...
async function runQueryPostgres(options, query) {
  const { connectionString } = options;
  const dbDefault = new import_pg.Client({
    connectionString
  });
